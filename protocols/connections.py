...
class Connections(module.Module):
    ...
    async def create_invitation(self, msg, agent):
        ...
        temp_connection.state = ConnectionState()
        temp_connection.state.role = Roles.INVITER
        temp_connection.state.transition(Events.SEND_INVITE)
        await agent.send_async({
            '@type': self.type('invitation'),
            'label': 'apts-demo-agent-as-inviter',
